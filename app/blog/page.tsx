import { getAllPost } from '../../lib/blog';
import Link from 'next/link';
import { BackButton } from './backButton';
import Image from 'next/image'

export default async function BlogPage() {
  const posts = getAllPost(); // can be sync if using fs
  console.log(posts);
  return (
    <section className="mx-auto py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8">
        {posts.map((post: any) => (
          ...
          >
            <h3 className="text-xl font-semibold text-accent hover:underline">{post.title}</h3>

            <p className="text-pink text-sm mt-2">{post.description}</p>
